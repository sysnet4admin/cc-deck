#!/usr/bin/env python3
"""cc-deck demo — fuzzy search + TODO/PIN + Tab mode cycle (English, README size)"""
import json, sys
...
# ── SCENE 0: TODO ─────────────────────────────────────────────────────────────
comment("0. ask Claude to track something → auto-pinned as TODO", pre=0.5, post=1.2)
...
out(f"\r\n{GRY}╰─{R}\r\n", 0.02); pause(0.5)
out(f"{GRY}╭─{R} Claude\r\n{GRY}│{R}  Recorded 1 memory {GRY}(ctrl+o to expand){R}\r\n{GRY}╰─{R}\r\n", 0.03)
pause(0.6)
out(f"\r\n{GRY}❯ /quit{R}\r\n", 0.03); pause(0.5)
...
pause(0.3)
out(f"{GRY}Claude Code{R} {GRN}v2.1.128{R} — resuming session\r\n{GRY}✓{R} applied 3Gi limit...\r\n", 0.03)
pause(1.0)

# ── SCENE 1: fuzzy search ─────────────────────────────────────────────────────
comment("1. fuzzy search — type to filter sessions instantly", pre=1.2, post=1.2)
...
pause(0.3)
out(f"{GRY}Claude Code{R} {GRN}v2.1.128{R} — resuming session\r\n{GRY}✓{R} pod keeps OOMKilling...\r\n", 0.03)
pause(1.0)

# ── SCENE 2: Tab mode cycling ─────────────────────────────────────────────────
comment("2. Tab cycles resume mode — shown in header", pre=1.2, post=1.2)
...
# ── SCENE 4: session size management ─────────────────────────────────────────
comment("4. large sessions surfaced below [Quick] — Ctrl-E trims, keeps recent turns", pre=1.2, post=0.8)
...
out_file = sys.argv[1] if len(sys.argv) > 1 else "demo_readme.cast"
